...
void calculatePhysics() {
	...
	while (running) {
		...
		if (GetAsyncKeyState(0x51))/*Q*/ { globalContainer.addRandomObjectAt(ImGui::GetMousePos(), 10, 10, 10); }
		if (GetAsyncKeyState(0x45))/*E*/ { globalContainer.addRandomObjectAt(ImGui::GetMousePos(), 30, 10); }
		...
		if (enableGravity && !GetAsyncKeyState(VK_INSERT)) {
			globalContainer.solveGravityAll();
		}

		globalContainer.solveVelocityAll();
		globalContainer.solveObjectsCollisionAll(8);
...
INT APIENTRY WinMain(HINSTANCE instance, HINSTANCE, PSTR, INT cmd_show) {
	...
	while (running) {
		...
		ImGui::Begin("controls", (bool*)0, ImGuiWindowFlags_NoMove);
