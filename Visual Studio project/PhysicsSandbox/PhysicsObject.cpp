...
PhysicsObject::PhysicsObject(Object obj, ImVec2 velocity) : Object(obj.mShape, obj.mPos, obj.mSize, obj.mColor) {
	mVelocity = velocity;
	mMass = obj.mSize * 5;
}

PhysicsObject PhysicsObject::createRandomPhysObj(RECT boundingBox, float size, int maxVel) {
...
PhysicsObject PhysicsObject::createRandomPhysObj(RECT boundingBox, float size, ImVec2 velocity) {
	return PhysicsObject(Object::createRandomCircle(boundingBox, size), velocity);
}

PhysicsObject PhysicsObject::createRandomPhysObj(RECT boundingBox, int deviationSize, int minSize, int maxVel) {
	float randVelX = rand() % maxVel * 2 - maxVel;
	float randVelY = rand() % maxVel * 2 - maxVel;
	ImVec2 randVelocity = { randVelX, randVelY };
...
void PhysicsObject::scale(float proportions) {
...
void PhysicsObject::invertVel(bool xy) {
	if (xy) {
...
void PhysicsObject::applyFriction(ImVec2 friction) {
	mVelocity = { mVelocity.x * friction.x, mVelocity.y * friction.y };
}

void PhysicsObject::solveGravity(float dt) {
	if (mGravity) {
		dt += 1;
...
void PhysicsObject::solveVelocity(float dt) {
	dt += 1;
	mPos.x = mPos.x + mVelocity.x * dt;
	mPos.y = mPos.y + mVelocity.y * dt;
}

void PhysicsObject::collideBox(RECT boxRect, int iterationCount, bool useApplyFriction) {
...
void PhysicsObject::setPosStatic(ImVec2 pos) {
	mPos = pos;
	mVelocity = { 0, 0 };
}

bool PhysicsObject::isColliding(Object& obj) {
...
void PhysicsObject::magnetTo(ImVec2 pos, float strength) {
...
void PhysicsObject::solveStaticCollide(PhysicsObject& obj1, PhysicsObject& obj2) {
...
void PhysicsObject::solveDynamicCollide(PhysicsObject& obj1, PhysicsObject& obj2, float efficiency) {
	float distance = sqrtf((obj2.mPos.x - obj1.mPos.x) * (obj2.mPos.x - obj1.mPos.x) + (obj2.mPos.y - obj1.mPos.y) * (obj2.mPos.y - obj1.mPos.y));
	if (distance == 0) {
